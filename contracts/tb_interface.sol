...
interface ITB_impl {
    ...
    function createBond(uint _initialSupply, uint32 _maturityDate, string memory _name, string memory _symbol, address _minter) external returns(uint, address);
    function depositBulk(DepositWithdrawalParams[20] calldata deposits) external;
    ...
    function transferBondAmongUsers(uint _bondId, uint _amount, address _receiver) external;
